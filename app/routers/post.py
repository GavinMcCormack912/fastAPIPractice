...
def make_post(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int= Depends(oauth2.get_current_user)):
    ...
    new_post = models.Post(owner_id=current_user.id, **post.dict()) #unpack the dictionary for sqlalchemy!
    db.add(new_post)
    db.commit()
    db.refresh(new_post)
    return new_post
...
@router.get("/{id}")  #path parameter to the id of the post a user wants to see
